...
- **鮮明**: 画像に緩やかなシャープ化フィルタを適用

### その他の機能
- **エディターの拡大/移動**: マウスホイールで最大 1200% まで拡大し、移動モードのドラッグまたはミドル/右ボタンドラッグで表示位置を移動
...
# 出力先: installer_output/SnapNoteStudio_Setup_2.1.0.exe
```
...
1. 左サイドバーで移動モードまたは注釈ツールを選択
2. 上部ツールバーで色、太さ、濃さを調整
3. 画像上に注釈を描画
4. マウスホイールで拡大/縮小し、移動モードのドラッグまたはミドル/右ボタンドラッグで表示位置を移動
5. `Ctrl+Z`で元に戻す、`Ctrl+Y`でやり直し
6. 「コピー」をクリックまたは`Ctrl+C`でクリップボードにコピー
7. 「保存」をクリックまたは`Ctrl+S`で設定した保存先へ保存
8. `Ctrl+X`、`Ctrl+W`、`Ctrl+Q`で終了を含む操作を実行
